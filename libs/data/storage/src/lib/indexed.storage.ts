...
export class IndexedStorage implements Storage {
  ...
  constructor(
    { dbName, storeName, dbVersion, noWrap }: IndexedConfig = {
      dbName: 'speekStorage',
          ...
          map((): unknown => {
            if (request.result !== undefined && request.result !== null) {
              ...
              if (
                ...
                this.wrapIndex in request.result &&
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
                request.result[this.wrapIndex] !== undefined &&
                request.result[this.wrapIndex] !== null
              ) {
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
  ...
  keys(): Observable<string> {
    ...
    return this.transaction('readonly').pipe(
      ...
      mergeMap((transactionData) => {
        ...
        const request =
          'openKeyCursor' in store
            ? store.openKeyCursor()
            : (store as IDBObjectStore).openCursor()
        ...
        const success$ = fromEvent(request, 'success').pipe(
          ...
          tap(() => {
            ;(request.result as IDBCursor).continue()
          })
        )
  ...
  has(key: string): Observable<boolean> {
    ...
    return this.transaction('readonly').pipe(
      mergeMap((transactionData) => {
        ...
        return events.pipe(
          map(() => (request.result !== undefined ? true : false))
        )
      }),
      /* The observable will complete */
      first()
  ...
  protected listenError(
    ...
    return fromEvent(transactionOrRequest, 'error').pipe(
      ...
      mergeMap((event) =>
        throwError((event.target as IDBTransaction | IDBRequest).error)
      )
    )
  }
   ...
   */
  protected listenTransactionEvents(
